...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: beforeBooking } = await supabaseAdmin
    .from('bookings')
    .select('daily_slot_id, flight_id')
    .eq('id', id)
    .single();
  ...
  const { data: afterBooking } = await supabaseAdmin
    .from('bookings')
    .select('daily_slot_id, flight_id')
    .eq('id', id)
    .single();
  ...
  if (
    beforeBooking
    && afterBooking
    && (beforeBooking.daily_slot_id !== afterBooking.daily_slot_id || beforeBooking.flight_id !== afterBooking.flight_id)
  ) {
    try {
      await sendBookingLifecycleEmail(id, 'reschedule');
    } catch (error) {
      console.error('reschedule_email_failed', error);
    }
  }
