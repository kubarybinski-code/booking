...
export async function GET(request: Request) {
  ...
  const slotsRes = await supabaseAdmin
    .from('daily_slots')
    .select('id, slot_date, slot_start, state, is_bookable, capacity, shared_capacity_key, flight_id, flights(slug)')
    .eq('slot_date', date)
    .order('slot_start');
...
export async function PATCH(request: Request) {
  ...
  const { error } = await supabaseAdmin.from('daily_slots').update({
    state: p.state,
  ...
  if (p.kind === 'manual-booking') {
    ...
    if (booking?.booking_id) {
      try {
        ...
        console.error('manual_confirmation_email_failed', error);
      }
    }
