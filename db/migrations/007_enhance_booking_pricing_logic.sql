...
begin
  ...
  for v_rule in
    select * from discount_rule_configs dr
    where dr.is_active = true
      and (array_length(dr.applicable_flight_ids,1) is null or p_flight_id = any(dr.applicable_flight_ids))
  loop
    v_auto_discount := v_auto_discount + resolve_discount_from_people_map(v_rule.people_discount_map, p_people_count, true);
  end loop;

  v_discount := least(v_subtotal, v_discount + v_auto_discount);
  ...
  values (v_new_booking_id, 'created', 'customer', jsonb_build_object('discount_code', p_discount_code, 'people_count', p_people_count));
